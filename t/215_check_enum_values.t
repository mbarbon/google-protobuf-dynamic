...

{
    ...
    eq_or_diff(
        Test1::MessageBefore->decode_to_perl(Test1::MessageAfter->encode_from_perl({ value => 3, array => [3, 2, 1] })),
...

{
    ...
    eq_or_diff(
        Test2::MessageBefore->decode_to_perl(Test2::MessageAfter->encode_from_perl({ value => 3, array => [3, 2, 1] })),
        Test2::MessageBefore->new({ value => 3, array => [3, 2, 1] }),
    );

    eq_or_diff(
        Test2::MessageBefore->encode_from_perl({ value => 3 }),
        "\x08\x03",
    );

    eq_or_diff(
        Test2::MessageBefore->encode_from_perl({ array => [3, 2] }),
        "\x10\x03\x10\x02",
    );
